        ...
        return function LocationManager(gameController, gameState) {
            ...
            this.relocateDown = function () {
                if (this.canRelocateDown()) {
                    this.gameState.location = data.locations[this.currentLocationIndex() - 1];
                    if (!this.gameState.location.availableContracts) this.gameState.location.availableContracts = [];
                    if (!this.gameState.location.availableHires) this.gameState.location.availableHires = [];
                    gameController.StatisticsManager().trackStat("relocate-to", this.gameState.location.name, 1);
                }
            };
            ...
            this.relocateUp = function () {
                if (this.canRelocateUp()) {
                    this.gameState.location = common.clone(data.locations[this.currentLocationIndex() + 1]);
                    if (!this.gameState.location.availableContracts) this.gameState.location.availableContracts = [];
                    if (!this.gameState.location.availableHires) this.gameState.location.availableHires = [];
                    gameController.StatisticsManager().trackStat("relocate-to", this.gameState.location.name, 1);
                }
