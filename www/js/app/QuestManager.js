    ...
    function QuestManager(
        ...
        return function QuestManager(gameController, gameState) {
            ...
            this.completeTasks = function (quest) {
                var completedTasks = tasks.filter(task => task.status == "in-progress" && task.finishes <= Date.now());
                completedTasks.forEach(function (task) {
                    // TODO Check for success, failure, injury, etc.
                    task.status = "Complete";
                    if (task.afterSuccessFinishQuest) {
                        this.finishQuest(quest);
                    } else {
                        this.startTask(quest, task.afterSuccessGoToTaskId);
                    }
                }, this);
            };

            this.finishQuest = function (quest) {
                this.completeQuest(quest);
            };

            this.startTask = function (quest, taskId) {
                var task = common.clone(quest.contract.tasks.filter(task => task.id = taskId)[0]);
                task.finishes = Date.now() + task.duration;
